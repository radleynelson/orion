...
open class TerminalView: UIScrollView, UITextInputTraits, UIKeyInput, UIScrollViewDelegate, TerminalDelegate, UIPointerInteractionDelegate, UIGestureRecognizerDelegate {
    ...
    {
        let scrollPan = panGestureRecognizer
        let gestureIsDiscrete = gestureRecognizer is UITapGestureRecognizer || gestureRecognizer is UILongPressGestureRecognizer
        let otherIsDiscrete = otherGestureRecognizer is UITapGestureRecognizer || otherGestureRecognizer is UILongPressGestureRecognizer

        if gestureRecognizer == scrollPan && otherIsDiscrete {
            return true
        }
        if otherGestureRecognizer == scrollPan && gestureIsDiscrete {
            return true
        }
        return false
    }
